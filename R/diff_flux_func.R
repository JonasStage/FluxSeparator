...
#' @importFrom ggplot2 geom_line theme_bw sec_axis theme ggplot
#' @importFrom stats coef na.exclude var
#' @importFrom utils data head write.csv
#' @import patchwork

diff_flux <- function(data, concentration_values = "pred_CH4", station, runvar_cutoff = 0.5, remove_observations_prior = 200,
  ...
  if(look_for_bubbles) {
    ...
    ids_to_remain_diff <- running_var_diff %>%
      ...
      group_by(gruppering) %>%
      pull(row) %>%
      { unique(c(outer(., seq(-IndexSpan, IndexSpan), `+`))) } %>%
      { .[. > 0] }

    running_var_diff %>%
      filter(!row %in% ids_to_remain_diff) -> remaining_data

     if(nrow(remaining_data) == 0) {
       statement = "ERROR! NO DATA"
       return(statement)
     } else {}

    bubbles_diff <- remaining_data %>%
      mutate(time_diff = datetime -lag(datetime),
             time_diff = as.numeric(time_diff)) %>%
      drop_na(time_diff) %>%
      mutate(gruppering =  1 + cumsum(time_diff>30)) %>%
    ...
    diffusive_dataset <- bubbles_diff %>%
      group_by(PumpCycle,station) %>%
  ...
  data_indelt <- data %>%
    ...
    plotting_data ->plot_lm
    data_indelt ->plot_raw

        ggplot() +
          geom_point(data = plot_raw, aes(datetime, concentration, group = PumpCycle)) +
          geom_point(data = plot_lm, aes(datetime, concentration, group = PumpCycle), col = "blue") +
        ...
        list_df <- list("diff" = diffusive_flux,
                        "plot" = p)
  return(list_df) }
